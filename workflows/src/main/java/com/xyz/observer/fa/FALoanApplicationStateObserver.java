...
class FALoanApplicationStateTrigger {
	...
	public FALoanApplicationStateTrigger(UniqueIdentifier loanApplicationId, LoanApplicationStatus applicationStatus,
			CordaRPCOps proxy) {
	...
	public void trigger() {
		try {
			...
			logger.info("Intiating Request to CA for credit check for LoanApplicationID : "
					+ loanApplicationId.toString() + " with status : " + loanApplicationStatus.toString());
			...
			SignedTransaction loanUpdateTx = proxy.startTrackedFlowDynamic(LoanApplicationCreationFlow.class,
					loanApplicationStatus, loanApplicationId, creditCheckApplicationId).getReturnValue().get();
			LoanApplicationState laState = ((LoanApplicationState) loanUpdateTx.getTx().getOutputs().get(0).getData());
			logger.info("Application status for the LoanApplication is updated LoanApplicationID: "
					+ laState.getLoanApplicationId().toString() + " CreditCheckApplicationId : "
