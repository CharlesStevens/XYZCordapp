...
public class FABankFinanceStateObserver {
    ...
    public FABankFinanceStateObserver(CordaRPCOps proxy, CordaX500Name name) {
        this.proxy = proxy;
        this.me = name;
    }
    ...
    public void observeBankFinanceState() {
        try {
            final DataFeed<Vault.Page<BankFinanceState>, Vault.Update<BankFinanceState>> bankStateFeed = proxy.vaultTrack(BankFinanceState.class);
            ...
            creditUpdates.toBlocking().subscribe(update -> update.getProduced().forEach(t -> {
                ...
                logger.info("Update in BankFinanceState detected for Bank Processing Id : " + bankFinanceState.getBankLoanProcessingId() + " with processing status : " +
                ...
                if (bankFinanceState.getBankProcessingStatus() != BankProcessingStatus.IN_PROCESSING) {
                    new FABankFinanceProcessedTrigger(bankLoanApplicationId, proxy, bankFinanceState.getBankProcessingStatus()).trigger();
                }
...
class FABankFinanceProcessedTrigger {
    ...
    public FABankFinanceProcessedTrigger(UniqueIdentifier bankProcessingId, CordaRPCOps proxy, BankProcessingStatus bankProcessingStatus) {
    ...
    public void trigger() {
        try {
            ...
            logger.error("Error while Processing BankProcessing Status for loanApplicationId : " + bankProcessingId.getId().toString());
            e.printStackTrace();
        }
    }
}
